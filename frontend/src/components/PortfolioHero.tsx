...
// Local shim — Wave 1 will add `useFxcmOrders` to data/hooks.ts; this
// inline fallback keeps the worktree typecheck-clean until merge.
const useFxcmOrdersShim = (enabled = true) =>
  useQuery({
    queryKey: ["fxcm", "orders"] as const,
    queryFn: async () => {
      const res = await fetch("/api/fxcm/orders");
      if (!res.ok) throw new Error(`fxcm/orders ${res.status}`);
...
function ForexPortfolioHero({ isMobile }: { isMobile: boolean }) {
  ...
  const orders = useFxcmOrdersShim(true);
