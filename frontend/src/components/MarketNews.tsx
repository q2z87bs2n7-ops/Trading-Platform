...
export default function MarketNews({ symbol }: { symbol?: string }) {
  ...
  const [tab, setTab] = useState<"market" | "instrument">("market");

  const showInstrumentTab = !!symbol;

  return (
    <div className="bg-panel border border-border rounded-lg p-3">
      <div className="flex items-center justify-between m-0 mb-2">
        <h2 className="text-[13px] uppercase tracking-wide text-muted m-0">
          {tab === "market" ? "Market News" : `News · ${symbol}`}
        </h2>
        {showInstrumentTab && (
          <div className="flex gap-1">
            <button
              type="button"
              className={`btn btn-mini${tab === "market" ? " active" : ""}`}
              style={{ opacity: tab === "market" ? 1 : 0.5 }}
            ...
            <button
              type="button"
              className={`btn btn-mini${tab === "instrument" ? " active" : ""}`}
              style={{ opacity: tab === "instrument" ? 1 : 0.5 }}
      ...
      {tab === "instrument" && symbol ? (
        <News symbol={symbol} bare />
      ) : (
        <>
